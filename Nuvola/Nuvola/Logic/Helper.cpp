...
void Helper::getUsersOnNetwork()
{
	...
	foundIpsFile.open("Logic\\foundIps.txt", ios::trunc);
	foundIpsFile.close();

	ofstream batFile;
	batFile.open("Logic\\GetLocalIps.bat", ios::trunc);
	batFile << "@echo off	\n";
	...
	batFile << "arp -a > Logic\\foundIps.txt";
	batFile.close();
	...
	LPCTSTR lpApplicationName = L"Logic\\GetLocalIps.bat";
	...
	cout << "Finished" << endl;
	getIpAddrsFromFile("Logic\\foundIps.txt");
}

string Helper::getBitsInSubnet(string subnet)
{
