...
void EncryptFileMain(std::string srcFile, std::string destFile, std::string password)
{
	...
	for (int i = 0; i < srcFile.length(); ++i)
		wideSrc += wchar_t(srcFile[i]);
	wchar_t* wSrc = const_cast<wchar_t*>(wideSrc.c_str());

	std::wstring wideDest;
	for (int i = 0; i < destFile.length(); ++i)
		wideDest += wchar_t(destFile[i]);
	wchar_t* wDest = const_cast<wchar_t*>(wideDest.c_str());

	std::wstring widePass;
	for (int i = 0; i < password.length(); ++i)
		widePass += wchar_t(password[i]);
	wchar_t* wPass = const_cast<wchar_t*>(widePass.c_str());

	LPTSTR pszSource = wSrc;
	LPTSTR pszDestination = wDest;
	LPTSTR pszPassword = wPass;
...
LPTSTR stringToLptstr(std::string str)
{
	...
	wchar_t* wtest = const_cast<wchar_t*>(wideStr.c_str());
	char buf[1024];
